/******
 * ABPlayer Core Debugger - Revised
 * Ver 1.05-20120326
 * Author : Jim Chen
...
function CommentManager(stageObject) {
	...
	this.initCmt = function(cmt, data) {
		...
		if (data.debug) {
			var debuggerText = document.createElement('div');
			debuggerText.style['float'] = 'right';
			debuggerText.style['cssFloat'] = 'right';
			debuggerText.appendChild(document.createTextNode('4000'));
			debuggerText.style.fontSize = "10px";
			debuggerText.style.color = "#ff0";
			debuggerText.id = "zDbg_" + this.zindex;
			cmt.appendChild(debuggerText);
			cmt.data.dbg = debuggerText;
			cmt.dbgId = "zDbg_" + this.zindex;
		}
...
CommentManager.prototype.onTimerEvent = function(timePassed, cmObj) {
	for (var i = 0; i < cmObj.runline.length; i++) {
		...
		cmt.ttl -= timePassed;
		if (cmt.mode == 1)
			cmt.style.left = (cmt.ttl / cmt.dur) * (cmObj.stage.offsetWidth + cmt.offsetWidth) - cmt.offsetWidth + "px";
		else if (cmt.mode == 6)
			cmt.style.left = (1 - cmt.ttl / cmt.dur) * (cmObj.stage.offsetWidth + cmt.offsetWidth) - cmt.offsetWidth + "px";
		...
		if (cmt.data.debug) {
			cmt.data.dbg.innerHTML = "" + cmt.ttl;
			//if(cmt.data.dbg.innerText != cmt.ttl + "")
			//	document.getElementById(cmt.dbgId).innerHTML = cmt.ttl;
		}
		if (cmt.ttl <= 0) {
			cmObj.stage.removeChild(cmt);
			cmObj.runline.splice(i, 1);
			//remove the comment
			cmObj.finish(cmt);
		}
	}
